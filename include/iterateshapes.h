...
static void getConfigurations(
    ...
    function<bool(const vector<int> &)> reject =
        [](const vector<int> &c) { return false; },
...
bool iterateSequentialGrid(vecn<int, N> &idx, const vecn<int, N> &steps,
                           function<bool(const vecn<int, N> &)> action,
                           int dim = 0) {
  if (dim >= idx.size()) {
    return action(idx);
  }
...
template <size_t N>
bool iterateGrid(int dim, int radius,
                 function<bool(const vecn<int, N> &)> action, int rad = 0) {
...
template <size_t N>
bool iterateStar(int dim, int radius,
                 function<bool(const vecn<int, N> &)> action, int rad = 0) {
